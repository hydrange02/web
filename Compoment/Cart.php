...
?>
        <div class="group bg-white rounded-xl shadow-sm border border-gray-100 mb-3 grid grid-cols-1 md:grid-cols-12 gap-4 p-4 items-center hover:shadow-md hover:border-blue-200 transition-all duration-300">
            ...
            <div class="col-span-1 md:col-span-1 text-center flex justify-center">
                <a href="../Config/DeleteCart.php?id=<?= $row['id'] ?>" class="w-9 h-9 rounded-full bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-red-100 hover:text-red-500 transition shadow-sm" onclick="return confirm('Xóa sản phẩm này?');"><i class="fas fa-trash-alt text-sm"></i></a>
            </div>
        </div>
<?php 
        endforeach; 
...
<script>
    ...
    async function updateCartQty(cartId, action) {
        ...
        try {
            ...
            const res = await fetch('../Config/UpdateCart.php', { method: 'POST', body: formData });
