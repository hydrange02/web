...
<body class="bg-slate-100 min-h-screen font-sans">
    ...
    <div class="ml-64 p-8 transition-all duration-300">
        ...
        <div id="add-form-container" class="hidden bg-white p-6 rounded-xl shadow-md mb-8 border border-blue-100">
            ...
            <form id="add-product-form" class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <input type="text" name="image" placeholder="Link ảnh (URL)" class="input-field col-span-2 md:col-span-1" required>
    ...
    <script>
        ...
        async function updateStock(id) {
            ...
            const data = await res.json();
            data.success ? Swal.fire({icon: 'success', title: 'Đã cập nhật kho', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500}) : Swal.fire('Lỗi', data.message, 'error');
        }

        // Delete Product
        async function deleteProduct(id) {
            if (await Swal.fire({title:'Xóa vĩnh viễn?', text:'Hành động này không thể hoàn tác!', icon:'warning', showCancelButton:true, confirmButtonColor:'#d33'}).then(r => r.isConfirmed)) {
